...
const Card = ({ trData, isTrending, index, media_type }) => {
  ...
  return (
    <Link to={"/" + mediaType + "/" + trData.id}>
      <div className="w-55 h-72 overflow-hidden rounded relative group cursor-pointer">
        <img src={!trData?.poster_path ? nullPic : imageUrl + trData?.poster_path} alt="shows" className=" hover:scale-105 transition-transform duration-300 w-full h-full object-cover " />
        ...
        <div className="absolute bottom-0 h-25 w-full bg-black/60 p-3 text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
          ...
          {trData.vote_average && <StarRating rating={trData.vote_average} />}

          {trData.release_date && <div className="text-sm mt-1">{moment(trData.release_date).format("MMM Do YYYY")}</div>}
        </div>
      </div>
    </Link>
  );
